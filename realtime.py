...
class PokerBot:
    ...
    def train_initial_model(self):
        # Dummy initial training data for example purposes
    ...
    def record_training_data(self, opponent_id, current_bet, pot, position, actual_action):
        model = self.opponent_models[opponent_id]
        feature = [model['aggressiveness'], model['bluffing_frequency'], current_bet, pot, position]
        self.features.append(feature)
        self.labels.append(actual_action)
    ...
    def make_decision(self, hand, community_cards, num_players, position, pot, current_bet, round_name, opponent_actions):
        ...
        for opponent_id, action in opponent_actions:
            ...
            # Map action strings to numerical labels
            action_mapping = {'fold': 0, 'call': 1, 'raise': 2}
            self.record_training_data(opponent_id, current_bet, pot, position, action_mapping[action])
